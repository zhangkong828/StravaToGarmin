...
namespace StravaToGarmin.Client.Strava
{
    public class StravaService
    {
        ...
        public async Task<bool> Login()
        {
            ...
            var postData = new StringContent($"utf8={utf8}&authenticity_token={authenticity_token.UrlEncode()}&plan=&email={email.UrlEncode()}&password={password}", new MediaTypeHeaderValue("application/x-www-form-urlencoded"));
        ...
        public async Task<List<Activity>> TrainingActivities(string keywords, string activity_type)
        {
            ...
            response.EnsureSuccessStatusCode().WriteRequestToConsole();
            var result = await response.Content.ReadAsStringAsync();
            var activities = result.JsonDeserializer<TrainingActivitiesResponse>();
